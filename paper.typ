...
其中$L$为有效弦长，$T$为张力，$mu$为线密度，$n$为模态阶数（$n=1$为基频）。该公式基于两端位移为零的边界条件。完整推导在文献中有详细给出，故此处略去推导。
...
工程上，DFT的朴素实现版本对于计算机来说处理开销很大。所以一般使用DFT的快速算法实现版本FFT，计算复杂度从$O(N^2)$降到$O(N log N)$。
...
其中$T_"rec"$为记录时长。Nyquist频率为$f_(N y q)=f_s/2$，采样定理要求$f_s>2f_max$以避免混淆。
...
计算机将离线处理获取的数据。即计算机不是实时处理接收到的信号，而是先存储数字信号文件，然后运行程序进行统一处理。程序由Python编写，CPython解释器运行，数学上使用NumPy的FFT实现。相关代码在GitHub上开源，并遵循MIT协议。
...
== 窗函数与频率刻度
为抑制谱泄露，使用Hann窗对选定段进行加窗。窗函数记为$omega[n]$，加窗后的信号为
